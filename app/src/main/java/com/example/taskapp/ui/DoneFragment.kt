        ...
        viewModel.taskList.observe(viewLifecycleOwner) { stateView ->
            when(stateView){
                ...
                is StateView.OnSuccess -> {
                    ...
                    val taskList = stateView.data?.filter { it.status == StatusEnum.DONE }
            ...
            when(stateView){
                ...
                is StateView.OnSuccess -> {
                    ...
                    val newList = oldList.toMutableList().apply {
                        if(!oldList.contains(stateView.data) && stateView.data?.status == StatusEnum.DONE){
                            add(0, stateView.data)
                            setPositionRecyclerView()
                        }

                        if (stateView.data?.status == StatusEnum.DONE) {
