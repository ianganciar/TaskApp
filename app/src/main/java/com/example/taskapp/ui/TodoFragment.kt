        ...
        viewModel.taskList.observe(viewLifecycleOwner) { stateView ->
            when (stateView) {
                ...
                is StateView.OnSuccess -> {
                    val taskList = stateView.data?.filter { it.status == StatusEnum.TODO }
        ...
        viewModel.taskInsert.observe(viewLifecycleOwner) { stateView ->
            when (stateView) {
                ...
                is StateView.OnSuccess -> {
                    ...
                    if (stateView.data?.status == StatusEnum.TODO) {
        ...
        viewModel.taskUpdate.observe(viewLifecycleOwner) { stateView ->
            when (stateView) {
                ...
                is StateView.OnSuccess -> {
                    ...
                    // Gera uma nova lista a partir da lista antiga já com a tarefa atualizada
                    val newList = oldList.toMutableList().apply {
                        if (!oldList.contains(stateView.data) && stateView.data?.status == StatusEnum.TODO) {
                            add(0, stateView.data)
                            setPositionRecyclerView()
                        }
                        if (stateView.data?.status == StatusEnum.TODO) {
